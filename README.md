...
**Traefik (file provider only):** **`./docker/traefik/traefik.yml`** loads **`providers.file.directory`** → **`./docker/traefik/dynamic.d/`** (all **`*.yml`** merged). **`./docker/traefik/ssl/`** is mounted at **`/etc/traefik/ssl`** for the **default** TLS certificate (**`cert.pem`** + **`key.pem`**). Run **`./scripts/ensure-traefik-ssl.sh`** once (also invoked from **`render-traefik-static.sh`** / **`setup.sh`**) to create a **self-signed** pair for local use; for production put your **full chain** in **`cert.pem`** (e.g. server cert + CA bundle) and the matching private key in **`key.pem`**. **MinIO** HTTPS routes in **`00-static.yml`** do **not** use **`tls.certResolver`** — they use the **default** store so a **commercial wildcard** (e.g. **`*.ptsi.co.id`**) covers **`s3.*`** / **`minio.*`** without hitting Let’s Encrypt’s **5 certs / 7 days per hostname** limit. **Managed** routes in **`01-managed.yml`** still use **`certResolver: letsencrypt`** unless you change the generator. See **`docker-compose.proxy.yml`** when attaching Traefik, **`web`**, and **`minio`** to an external **`proxy`** network. If Traefik returns **404**, re-run **`./scripts/render-traefik-static.sh`**, **Regenerate**, check **DNS** / **`.env`**, then **`docker compose up -d --force-recreate traefik`**.
...
**Traefik v3.6 + HTTP-01:** Traefik answers **`/.well-known/acme-challenge/*`** on **`web`** via its **internal** handler — do **not** add a manual router for that path (and never point it at **`noop@internal`**, which does not serve the token). **`00-static.yml`** (MinIO) and generated **`01-managed.yml`** use plain **`Host()`** rules and **`priority: 1`** on file-provider routers so **`redirect-https`** on **:80** does not win over the internal challenge router (higher numeric priority wins). Re-run **`./scripts/render-traefik-static.sh`**, **Regenerate** managed routes, then recreate **`traefik`** after pulling.
