...
- **Telegram** tokens are never generated (they must come from BotFather). **RBAC seed users** are **static** in **`packages/db/prisma/seeds/rbac.seed.ts`** (fixed emails under **`edgecontrol.local`** and one shared first-boot password); run **`./scripts/setup.sh seed`** to (re)apply them to the database
