...
**Traefik (file + Docker providers):** **static** config is **`./docker/traefik/traefik.yml`** (file provider). **`docker-compose.yml`** adds **`--providers.docker.*`** so the attach network is **`${COMPOSE_PROJECT_NAME:-edgecontrol}_internal`**. **Labels** on **`web`**, **`api`**, and **`minio`** define HTTPS routers; **`/var/run/docker.sock`** is mounted read-only. **`ACME_EMAIL`** and **`DOCKER_API_VERSION`** are set on the Traefik container. **`dynamic.yml`** (API-managed) merges with Docker routes. Until the API writes routes, **`dynamic.yml`** can stay **`{}`**; the panel still works on **`PANEL_HOST`** via labels if **DNS** and **`.env`** match.
