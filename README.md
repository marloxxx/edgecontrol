...
**Traefik (file + Docker providers):** **`./docker/traefik/traefik.yml`** defines **both** `providers.file` and `providers.docker` (do not rely on CLI `--providers.docker` alone: if the static file already has a `providers` block, those flags are easy to ignore, so Docker never starts and labelled hosts **404**). Compose sets **`DOCKER_INTERNAL_NETWORK`** to **`${COMPOSE_PROJECT_NAME}_internal`** (e.g. **`edgecontrol_internal`**) for **`network:`** in the YAML. **Labels** on **`web`**, **`api`**, **`minio`**; **`docker.sock`** read-only; **`ACME_EMAIL`**, **`DOCKER_API_VERSION`**. If **`https://edgecontrol.your-domain`** still 404s, check **`BASE_DOMAIN`** / **`PANEL_HOST`** in **`.env`** match the browser host exactly, then **`docker compose up -d --force-recreate traefik web`**.
