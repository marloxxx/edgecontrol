...
Monorepo for a **Traefik-centric operations panel**: manage dynamic routing configuration, monitor service health, and enforce RBAC. The API (NestJS) writes **`docker/traefik/dynamic.d/01-managed.yml`**; **`00-static.yml`** (from **`.env`**) defines **MinIO** Traefik routes only — expose the **panel** via **managed** routes or loopback; a worker processes background jobs (BullMQ / Redis); the web app is built with Vite and React.
...
**Traefik (file provider only):** **`./docker/traefik/traefik.yml`** loads **`providers.file.directory`** → **`./docker/traefik/dynamic.d/`** (all **`*.yml`** merged). **`./docker/traefik/ssl/`** is mounted at **`/etc/traefik/ssl`** for the **default** TLS certificate (**`cert.pem`** + **`key.pem`**). Run **`./scripts/ensure-traefik-ssl.sh`** once (also invoked from **`render-traefik-static.sh`** / **`setup.sh`**) to create a **self-signed** pair for local use; for production put your **full chain** in **`cert.pem`** (e.g. server cert + CA bundle) and the matching private key in **`key.pem`**. **MinIO** HTTPS routes in **`00-static.yml`** do **not** use **`tls.certResolver`** — they use Traefik’s **default** TLS store (**`docker/traefik/ssl/cert.pem`** + **`key.pem`**), e.g. a **commercial wildcard** for **`*.ptsi.co.id`**, so you do not hit Let’s Encrypt’s **5 certificates / 7 days per hostname** limit on **`s3.*`** / **`minio.*`**. **Managed** routes in **`01-managed.yml`** follow the same rule (**no** **`certResolver`** on **`websecure`**): put the same wildcard (full chain + key) in **`ssl/`** so every managed **FQDN** under that wildcard is covered. Use **`ACME_CA_SERVER`** / per-route ACME only if you later add an explicit opt-in in the product. See **`docker-compose.proxy.yml`** when attaching Traefik, **`web`**, and **`minio`** to an external **`proxy`** network. If Traefik returns **404**, re-run **`./scripts/render-traefik-static.sh`**, **Regenerate**, check **DNS** / **`.env`**, then **`docker compose up -d --force-recreate traefik`**.
...
**404 on :443:** if **Host** does not match **`00-static.yml`** (MinIO) or **`01-managed.yml`**, Traefik returns **404**. Set **`BASE_DOMAIN`** / **`MINIO_*_HOST`** and **`PANEL_HOST`** (for CORS / managed domain), run **`./scripts/render-traefik-static.sh`**, recreate **`traefik`**.
...
**`NET::ERR_CERT_AUTHORITY_INVALID` (Chrome) on `https://…ptsi.co.id`:** Traefik’s **default** TLS store reads **only** **`docker/traefik/ssl/cert.pem`** and **`key.pem`**. Putting **`STAR_*.crt`** / **`.ca-bundle`** in the same folder does **nothing** until you build **`cert.pem`** as the **full chain** (server + intermediates) and **`key.pem`** as the **matching private key**. If **`./scripts/ensure-traefik-ssl.sh`** ran earlier, it may have created a **self-signed** **`cert.pem`** (`CN=edgecontrol-traefik-local`) — that file blocks the script from recreating material and **browsers will not trust it**. Replace those two files, then **`docker compose up -d --force-recreate traefik`**. To see what is served: `openssl s_client -connect <host>:443 -servername edgecontrol.ptsi.co.id </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer`.
...
3. **Dynamic files present** — `docker exec edgecontrol-traefik ls -la /etc/traefik/dynamic.d` — expect **`00-static.yml`** (from **`./scripts/render-traefik-static.sh`**) and **`01-managed.yml`** after you **Regenerate** in the UI (or the API deletes **`01-managed.yml`** when there are no enabled managed services).
...
|--------|----------------------|-------------------------|
| Panel / API vs managed edge | **Loopback** ports and/or **Traefik :443** for the panel via **`01-managed.yml`**; MinIO hostnames in **`00-static.yml`**; API stays internal (**nginx** → **`http://api:3000`**). | Same ideas; split Traefik to its own host if you prefer. |
